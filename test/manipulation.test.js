...
describe("manipulation", function () {
	...
	describe("create", function () {
		...
		it("should create instance (promise variant)", function (done) {
			Person.create({ name: "Anatoliy" })
				.then(function (p) {
					p.name.should.equal("Anatoliy");
					should.exist(p);
					return Person.findById(p.id).then(function (person) {
						person.id.should.eql(p.id);
						person.name.should.equal("Anatoliy");
						done();
					});
		...
		it("should create batch of objects (promise variant)", function (done) {
			const batch = [{ name: "ShaltayPromise" }, { name: "BoltayPromise" }, {}];
		...
		it("should create batch of objects with beforeCreate", function (done) {
			...
			const batch = [{ name: "A" }, { name: "B" }, undefined];
		...
		bdd.itIf(
			connectorCapabilities.refuseDuplicateInsert !== false,
			...
			function (done) {
				...
				db.automigrate("ProductTest", function (err) {
					...
					Product.create({ name: "a-name" }, function (err, p) {
						if (err) return done(err);
						Product.create(
							{ id: p.id, name: "duplicate" },
							function (err, result) {
								if (!err) {
									return done(
										new Error("Create should have rejected duplicate id."),
									);
								}
	...
	describe("save", function () {
		...
		bdd.itIf(
			connectorCapabilities.cloudantCompatible !== false,
			"should save existing object (promise variant)",
			function (done) {
				// Cloudant could not guarantee findOne always return the same item
				Person.findOne()
					.then(function (p) {
						...
						return p.save().then(function () {
							return Person.findOne().then(function (p) {
			...
			Person.findOne()
				.then(function (p) {
					...
					return p.save().then(
						...
						function (err) {
							should.exist(err);
							p.save({ validate: false }).then(function (d) {
								should.exist(d);
								done();
							});
						},
	...
	describe("updateAttributes", function () {
		...
		it("should have updated password hashed with updateAttribute", function (done) {
			StubUser.create({ password: "foo" }, function (err, created) {
				if (err) return done(err);
		...
		it("should update one attribute (promise variant)", function (done) {
			person
				.updateAttribute("name", "Teddy Graham")
				.then(function (p) {
					return Person.all().then(function (ps) {
						ps.should.have.lengthOf(1);
		...
		bdd.itIf(
			...
			function (done) {
				...
				Person.findById(person.id, function (err, p) {
					if (err) return done(err);
					p.updateAttributes(
						{ name: "John", unknownVar: undefined },
						function (err, p) {
		...
		it("should allow unknown attributes when strict: false", function (done) {
			Person.definition.settings.strict = false;
			Person.findById(person.id, function (err, p) {
				if (err) return done(err);
				p.updateAttributes({ name: "John", foo: "bar" }, function (err, p) {
					if (err) return done(err);
		...
		it("should remove unknown attributes when strict: filter", function (done) {
		...
		it("should return error on unknown attributes when strict: true", function (done) {
		...
		it("should fallback to strict:true when using strict: throw", function (done) {
			...
			Person.findById(person.id, function (err, p) {
				if (err) return done(err);
				p.updateAttributes({ foo: "bar" }, function (err, p) {
					should.exist(err);
					err.name.should.equal("ValidationError");
					err.message.should.containEql("`foo` is not defined in the model");
		...
		it("should allow same stringified id value on updateAttributes", function (done) {
			let pid = person.id;
			if (typeof person.id === "object" || typeof person.id === "number") {
				// For example MongoDB ObjectId
				pid = person.id.toString();
			}
			person.updateAttributes({ id: pid, name: "John" }, function (err, p) {
				if (err) return done(err);
				Person.findById(p.id, function (e, p) {
					if (e) return done(e);
					p.name.should.equal("John");
		...
		it("should fail if an id value is to be changed on updateAttributes", function (done) {
			person.updateAttributes(
				{ id: person.id + 1, name: "John" },
				function (err, p) {
					should.exist(err);
					done();
				},
		...
		it("should allow model instance on updateAttributes", function (done) {
			person.updateAttributes(
				new Person({ name: "John", age: undefined }),
				function (err, p) {
					if (err) return done(err);
					Person.findById(p.id, function (e, p) {
						if (e) return done(e);
		...
		it("should allow model instance on updateAttributes (promise variant)", function (done) {
			person
				.updateAttributes(new Person({ name: "Jane", age: undefined }))
				.then(function (p) {
					return Person.findById(p.id).then(function (p) {
	...
	describe("updateOrCreate", function () {
		...
		it("should reject updated empty password with updateOrCreate", function (done) {
			StubUser.create({ password: "abc123" }, function (err, createdUser) {
				if (err) return done(err);
				StubUser.updateOrCreate(
					{ id: createdUser.id, password: "" },
					function (err, updatedUser) {
						err.message.should.match(/password cannot be empty/);
						done();
					},
		...
		it("preserves empty values from the database", async () => {
			...
			const found = await Player.updateOrCreate({
				id: created.id,
				name: "updated",
			});
	...
	bdd.describeIf(
		connectorCapabilities.supportForceId !== false,
		...
		function () {
			...
			it("fails when id does not exist in db & validate is false when using updateAttributes", function (done) {
				const unknownId = uid.fromConnector(db) || 123;
				const post = new Post({ id: unknownId });
				post.updateAttributes(
					{ title: "updated title", content: "AAA" },
					{ validate: false },
					(err) => {
						should(err).have.property("statusCode", 404);
						done();
			...
			it("works without options on create (promise variant)", function (done) {
				...
				Post.replaceOrCreate(post)
					.then(function (p) {
						...
						p.title.should.equal(post.title);
						p.content.should.equal(post.content);
						return Post.findById(p.id).then(function (p) {
							p.id.should.eql(post.id);
			...
			it("works without options on update (promise variant)", function (done) {
				...
				Post.create(post)
					.then(function (created) {
						...
						created.title = "b";
						return Post.replaceOrCreate(created).then(function (p) {
							should.exist(p);
							p.should.be.instanceOf(Post);
							...
							return Post.findById(created.id).then(function (p) {
								p.should.not.have.property("_id");
								p.title.should.equal("b");
			...
			it("works with options on update (promise variant)", function (done) {
				...
				Post.create(post)
					.then(function (created) {
						...
						delete created.content;
						created.title = "b";
						return Post.replaceOrCreate(created, { validate: false }).then(
							function (p) {
								should.exist(p);
								p.should.be.instanceOf(Post);
								p.id.should.eql(created.id);
	...
	bdd.describeIf(
		hasReplaceById && connectorCapabilities.supportForceId !== false,
	...
	if (!hasReplaceById) {
		...
		describe("replaceAttributes", function () {
			...
			it("should have updated password hashed with replaceAttributes", function (done) {
				StubUser.create({ password: "foo" }, function (err, created) {
					if (err) return done(err);
					created.replaceAttributes(
						{ password: "test" },
						function (err, created) {
							if (err) return done(err);
							created.password.should.equal("test-TEST");
			...
			it("should set cannotOverwritePKInBeforeSaveHook flag, if `instance` in" +
				"`before save` operation hook is set, so we report a warning just once", function (done) {
				Post.findById(postInstance.id, function (err, p) {
					if (err) return done(err);
					changePostIdInHook("before save");
					p.replaceAttributes({ title: "b" }, function (err, data) {
			...
			it("works without options(promise variant)", function (done) {
				Post.findById(postInstance.id)
					.then(function (p) {
						p.replaceAttributes({ title: "b" }).then(function (p) {
							should.exist(p);
							p.should.be.instanceOf(Post);
							p.title.should.equal("b");
							p.should.have.property("content").be.oneOf(null, undefined);
							return Post.findById(postInstance.id).then(function (p) {
								p.title.should.equal("b");
								should.not.exist(p.content);
								done();
							});
			...
			it("works with options(promise variant)", function (done) {
				Post.findById(postInstance.id)
					.then(function (p) {
						p.replaceAttributes({ title: "b" }, { validate: false }).then(
							function (p) {
								should.exist(p);
								p.should.be.instanceOf(Post);
								p.title.should.equal("b");
				...
				Post.findById(postInstance.id, function (err, p) {
					...
					p.replaceAttributes({ title: "b", id: unknownId }, function (err, p) {
						...
						const expectedErrMsg =
							"id property (id) cannot be updated from " +
							postInstance.id +
							" to " +
							unknownId;
	...
	bdd.describeIf(hasReplaceById, "replaceById", function () {
		...
		bdd.itIf(
			connectorCapabilities.supportForceId !== false,
			"fails when id does not exist in db " + "using replaceById",
			function (done) {
				const unknownId = uid.fromConnector(db) || 123;
				const post = { id: unknownId, title: "a", content: "AAA" };
	...
	describe("findOrCreate", function () {
		...
		it("preserves empty values from the database", async () => {
			...
			const [found] = await Player.findOrCreate(
				{ id: created.id },
				{ name: "updated" },
			);
			should(found.toObject().active).be.oneOf([
	...
	describe("destroy", function () {
			...
			Person.create()
				.then(function (p) {
					return p.destroy().then(function () {
						return Person.exists(p.id).then(function (ex) {
							ex.should.not.be.ok;
							done();
						});
					});
		...
		it("should destroy all records (promise variant)", function (done) {
			Person.create()
				.then(function () {
					return Person.destroyAll().then(function () {
						return Person.all().then(function (ps) {
							ps.should.have.lengthOf(0);
							return Person.count().then(function (count) {
								count.should.eql(0);
								done();
							});
						});
	...
	bdd.describeIf(
		connectorCapabilities.reportDeletedCount !== false &&
		...
		function () {
			...
			it("should report zero deleted instances when no matches are found", function (done) {
			...
			it("should delete all instances when the where condition is not provided", function (done) {
	...
	describe("deleteById", function () {
		...
		it("should allow deleteById(id) - fail with error", function (done) {
			const unknownId = uid.fromConnector(db) || 9999;
			const errMsg =
				"No instance with id " + unknownId.toString() + " found for Person";
	...
	describe("prototype.delete", function () {
		...
		bdd.itIf(
			connectorCapabilities.supportStrictDelete !== false,
			"should allow delete(id) - " + "fail with error",
			function (done) {
				Person.settings.strictDelete = true;
					...
					u.delete(function (err, info) {
						...
						u.delete(function (err) {
							...
							err.message.should.equal(
								"No instance with id " + u.id + " found for Person",
							);
							err.should.have.property("code", "NOT_FOUND");
							err.should.have.property("statusCode", 404);
							done();
	...
	describe("initialize", function () {
		...
		describe("Date $now function (type: Date)", function () {
			...
			it("should report current date as default value for date property", function (done) {
				const now = Date.now();
		...
		describe("Date $now function (type: String)", function () {
			...
			it("should report '$now' as default value for string property", function (done) {
				CustomModel.create(function (err, model) {
					if (err) return done(err);
		...
		describe("uuidv7 defaultFn", function () {
			let CustomModel;

			before(function (done) {
				CustomModel = db.define("CustomModel5", {
					guid: { type: String, defaultFn: "uuidv7" },
				});
				db.automigrate("CustomModel5", done);
			});

			it('should generate a new id when "defaultfn" is "uuidv7"', function (done) {
				CustomModel.create(function (err, model) {
					should.not.exists(err);
					model.guid.should.match(UUID_REGEXP);
					done();
	...
	describe("update/updateAll", function () {
		...
		it("should not update instances that do not satisfy the where condition", function (done) {
			idHarry = uid.fromConnector(db) || undefined;
			const filter =
				connectorCapabilities.updateWithOtherThanId === false
					? { id: idHarry }
					: { name: "Harry Hoe" };
			Person.update(filter, { name: "Marta Moe" }, function (err, info) {
				if (err) return done(err);
				if (connectorCapabilities.reportDeletedCount !== false) {
					info.should.have.property("count", 0);
				} else {
		...
		it("should only update instances that satisfy the where condition", function (done) {
			const filter =
				connectorCapabilities.deleteWithOtherThanId === false
					? { id: idBrett }
					: { name: "Brett Boe" };
			Person.update(filter, { name: "Harry Hoe" }, function (err, info) {
				if (err) return done(err);
				if (connectorCapabilities.reportDeletedCount !== false) {
					info.should.have.property("count", 1);
				} else {
		...
		bdd.itIf(
			connectorCapabilities.updateWithoutId !== false,
			"should update all instances when the where condition is not provided",
			function (done) {
				filterHarry =
					connectorCapabilities.deleteWithOtherThanId === false
						? { id: idHarry }
		...
		bdd.itIf(
			...
			"should ignore where " + "conditions with undefined values",
			function (done) {
				Person.update(
					filterBrett,
					{ name: undefined, gender: "male" },
					function (err, info) {
						if (err) return done(err);
						info.should.have.property("count", 1);
						Person.find({ where: filterBrett }, function (err, people) {
	...
	describe("upsertWithWhere", function () {
		...
		it("works without options on update (promise variant)", function (done) {
			...
			Person.create(person)
				.then(function (created) {
					...
					delete created.city;
					created.name = "BBB";
					return Person.upsertWithWhere({ id: 456 }, created).then(
						function (p) {
							should.exist(p);
							p.should.be.instanceOf(Person);
							p.id.should.eql(created.id);
							...
							return Person.findById(created.id).then(function (p) {
								p.should.not.have.property("_id");
								p.name.should.equal("BBB");
		...
		it("works with options on update (promise variant)", function (done) {
			...
			Person.create(person)
				.then(function (created) {
					...
					return Person.upsertWithWhere({ id: 789 }, created, {
						validate: false,
					}).then(function (p) {
						should.exist(p);
						p.should.be.instanceOf(Person);
		...
		it("creates a new record when no matching instance is found", function (done) {
			Person.upsertWithWhere(
				{ city: "Florida" },
				{ name: "Nick Carter", id: 1, city: "Florida" },
				function (err, created) {
					if (err) return done(err);
		...
		bdd.itIf(
			connectorCapabilities.atomicUpsertWithWhere !== true,
			"fails the upsertWithWhere operation when multiple instances are " +
				"retrieved based on the filter criteria",
			function (done) {
				Person.create(
					[
					...
					function (err, instance) {
						...
						Person.upsertWithWhere(
							...
							function (err) {
								err.message.should.equal(
									"There are multiple instances found." +
		...
		it("preserves empty values from the database", async () => {
			...
			const found = await Player.upsertWithWhere(
				{ id: created.id },
				{ name: "updated" },
			);
			should(found.toObject().active).be.oneOf([
