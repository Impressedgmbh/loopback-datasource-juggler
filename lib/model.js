...
ModelBaseClass.prototype._initProperties = function (data, options) {
	...
	if (
		...
		throw new Error(
			g.f(
				'Property name "{{constructor}}" is not allowed in %s data',
				ctor.modelName,
			),
		);
	}
	...
	if (strict === undefined) {
		...
		g.warn(
			"Warning: Model %s, {{strict mode: `throw`}} has been removed, " +
				"please use {{`strict: true`}} instead, which returns" +
				"{{`Validation Error`}} for unknown properties,",
			ctor.modelName,
		);
	}

	const persistUndefinedAsNull =
		ctor.definition.settings.persistUndefinedAsNull;
        ...
        */
			...
			if (relationType === "belongsTo" && propVal != null) {
				...
				self.__data[ctor.relations[p].keyFrom] =
					propVal[ctor.relations[p].keyTo];

				if (ctor.relations[p].options.embedsProperties) {
					const fields = fieldsToArray(
						ctor.relations[p].properties,
						modelTo.definition.properties,
						modelTo.settings.strict,
					);
	...
	for (let k = 0; k < size; k++) {
		...
		if (
			applyDefaultValues &&
			propVal === undefined &&
			appliesDefaultsOnWrites(properties[p])
		) {
		...
		if (
			ignoresMatchedDefault(properties[p]) &&
			properties[p].default === propVal
		) {
		...
		if (applyDefaultValues && propVal === undefined) {
			...
			switch (defn) {
				...
				case "guid":
				case "uuid":
					// Generate a RFC4122 v4 id
					propVal = uuid[`v${process.env.IWS_POSTGRES_UUID_VERSION ? process.env.IWS_POSTGRES_UUID_VERSION : "4"}`]();
					break;
				case "uuidv4":
			...
			// FIXME: We should coerce the value
			// will implement it after we refactor the PropertyDefinition
			if (propVal !== undefined) self.__data[p] = propVal;
		}
...
ModelBaseClass.getPropertyType = function (propName) {
	...
	if (!prop.type) {
		throw new Error(
			g.f("Type not defined for property %s.%s", this.modelName, propName),
		);
		// return null;
	}
 ...
 */
ModelBaseClass.prototype.toObject = function (
	onlySchema,
	removeHidden,
	removeProtected,
) {
	...
	const persistUndefinedAsNull =
		Model.definition.settings.persistUndefinedAsNull;
	...
	if (schemaLess) {
		...
		for (let i = 0; i < size; i++) {
			...
			if (data[propertyName] === undefined) {
				...
				val =
					ownVal !== undefined && typeof ownVal !== "function"
						? ownVal
						: self.__data[propertyName];
				if (typeof val === "function") {
					continue;
				}
...
ModelBaseClass.isProtectedProperty = function (propertyName) {
	...
	const protectedProperties =
		settings.protectedProperties || settings.protected;
...
ModelBaseClass.prototype.reset = function () {
	...
	for (const k in obj) {
		if (
			k !== "id" &&
			!obj.constructor.dataSource.definitions[obj.constructor.modelName]
				.properties[k]
		) {
			delete obj[k];
		}
...
const versionParts =
	process.versions && process.versions.node
		? process.versions.node.split(/\./g).map(function (v) {
				return +v;
			})
...
if (util.inspect.custom) {
	...
	ModelBaseClass.prototype[util.inspect.custom] =
		ModelBaseClass.prototype.inspect;
}
