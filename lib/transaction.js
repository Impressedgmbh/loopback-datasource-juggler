	...
	if (Transaction) {
		...
		Transaction.begin(connector, options, function (err, transaction) {
			...
			if (!transaction.id) {
				...
				transaction.id = uuid[`v${process.env.IWS_POSTGRES_UUID_VERSION ? process.env.IWS_POSTGRES_UUID_VERSION : '4'}`]();
			}
					...
					transaction.notifyObserversOf("timeout", context, function (err) {
						if (!err) {
							transaction.rollback(function () {
								debug(
									"Transaction %s is rolled back due to timeout",
									transaction.id,
								);
							});
						}
